...
func TestNewManager(t *testing.T) {
	tests := map[string]struct {
		...
		"Creates all brokers when config dir has only valid brokers": {
			brokerConfigDir: "valid_brokers",
		},
		"Creates without autodiscovery when configuredBrokers is set": {
			brokerConfigDir: "valid_brokers", configuredBrokers: []string{"valid_2.conf"},
		},
		"Creates only correct brokers when config dir has valid and invalid brokers": {
			brokerConfigDir: "mixed_brokers",
		},
		"Creates only local broker when config dir has only invalid ones": {
			brokerConfigDir: "invalid_brokers",
		},
		"Creates only local broker when config dir does not exist": {
			brokerConfigDir: "does/not/exist",
		},
		"Creates manager even if broker is not exported on dbus": {
			brokerConfigDir: "not_on_bus",
		},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			got, err := brokers.NewManager(context.Background(),
				filepath.Join(brokerConfFixtures, tc.brokerConfigDir), tc.configuredBrokers)
			if tc.wantErr {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			require.Equal(t, want.ID, got.ID,
				"SetDefaultBrokerForUser should have assiged the expected broker, but did not")
		})
	}
}
...
func TestBrokerFromSessionID(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			require.Equal(t, tc.wantBrokerID, got.ID,
				"BrokerFromSessionID should return the expected broker, but did not")
		})
	}
}
...
func TestNewSession(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Successfully start a new session with the correct broker": {
			username: "success", configuredBrokers: []string{t.Name() + "_Broker1.conf", t.Name() + "_Broker2.conf"},
		},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			require.Equal(t, wantBroker.ID, gotBroker.ID,
				"BrokerFromSessionID should have assigned the expected broker for the session, but did not")
		})
	}
}
...
func TestEndSession(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Successfully end session on the correct broker": {
			sessionID: "success", configuredBrokers: []string{t.Name() + "_Broker1", t.Name() + "_Broker2"},
		},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			require.Error(t, err,
				"EndSession should have removed the broker from the active transactions, but did not")
		})
	}
}
...
func TestStartAndEndSession(t *testing.T) {
	...
	require.Equal(t, b1.Name, assignedBroker.Name,
		"First NewSession should have assigned the expected broker for the session, but did not")
	...
	require.Equal(t, b2.Name, assignedBroker.Name,
		"Second NewSession should have assigned the expected broker for the session, but did not")
