...
func (s Service) GetUserByName(ctx context.Context, req *authd.GetUserByNameRequest) (*authd.User, error) {
	...
	user, err = s.userPreCheck(ctx, req.GetName())
	if err != nil {
...
// GetGroupByGID returns the group entry for the given group ID.
func (s Service) GetGroupByGID(ctx context.Context, req *authd.GetGroupByIDRequest) (*authd.Group, error) {
	if req.GetId() == 0 {
