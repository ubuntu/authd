...
// DisablePasswd marks a user as disabled
func (s Service) DisablePasswd(ctx context.Context, req *authd.DisablePasswdRequest) (*authd.Empty, error) {
	if err := s.permissionManager.IsRequestFromRoot(ctx); err != nil {
		return nil, err
	}
...
// EnablePasswd marks a user as enabled
func (s Service) EnablePasswd(ctx context.Context, req *authd.EnablePasswdRequest) (*authd.Empty, error) {
	if err := s.permissionManager.IsRequestFromRoot(ctx); err != nil {
		return nil, err
	}
