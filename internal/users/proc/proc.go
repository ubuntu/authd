...
func CheckUserBusy(name string, uid uint32) error {
	...
	for _, entry := range entries {
		...
		if err != nil {
			log.Debugf(context.Background(), "Skipping invalid entry %q: %v", entryName, err)
			continue
		}
		log.Debugf(context.Background(), "Checking process %d", pid)
		...
		if err := syscall.Stat(rootPath, &procRootStat); err != nil {
			log.Debugf(context.Background(), "Skipping invalid root path %q: %v", rootPath, err)
			continue
		}
		if rootStat.Dev != procRootStat.Dev || rootStat.Ino != procRootStat.Ino {
			log.Debugf(context.Background(), "Skipping process %d in different mount namespace", pid)
			continue
		}
...
func checkStatus(username, sname string, uid uint32) bool {
	...
	for scanner.Scan() {
		line := scanner.Text()
		log.Debugf(context.Background(), "Checking line: %s", line)
