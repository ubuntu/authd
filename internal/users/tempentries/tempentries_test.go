...
func TestLockedInvalidActions(t *testing.T) {
	...
	require.Panics(t, func() { _, _ = (&TemporaryRecordsWithLock{}).RegisterPreAuthUser("foobar") },
		"RegisterPreAuthUser should panic but did not")
	require.Panics(t, func() { _, _, _ = (&TemporaryRecordsWithLock{}).RegisterUser("foobar") },
		"RegisterUser should panic but did not")
	require.Panics(t, func() { _, _, _ = (&TemporaryRecordsWithLock{}).RegisterGroupForUser(123, "foobar") },
		"RegisterGroupForUser should panic but did not")

	require.Panics(t, func() { NewTemporaryRecords(nil).LockForChanges(&localentries.UserDBLocked{}) },
		"LockForChanges should panic but did not")

	tmpRecords := NewTemporaryRecords(&idgenerator.IDGenerator{UIDMax: 100})

	entries, entriesUnlock, err := localentries.NewUserDBLocked(([]localentries.Option{})...)
