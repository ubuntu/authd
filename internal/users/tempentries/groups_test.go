...
func TestRegisterGroup(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Successfully_registering_a_group_with_the_same_name_returning_the_same_GID": {
			groups:   []string{defaultGroupName, defaultGroupName},
			wantGIDs: []uint32{gidToGenerate, gidToGenerate},
		},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			for idx, groupName := range tc.groups {
				...
				if !slices.Contains(tc.groups[idx+1:], groupName) {
					require.Error(t, err, "GroupByID should return an error, but did not")
					continue
				}

				require.NoError(t, err, "GroupByID should return an error, but did not")
			}
		})
	}
}
