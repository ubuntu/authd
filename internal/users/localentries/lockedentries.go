...
type options struct {
	...
	// reads and writes on the group fill.
	// We need this to ensure that we don't write to the file while we're
	// parsing it.
	// The mutex is tied to the options since it's where different file paths
	// can be defined, and avoids us to have a shared global mutex when they
...
func NewWithLock(args ...Option) (entries *WithLock, cleanup func() error, err error) {
	...
	localGroupsMu := defaultOptions.localGroupsMu
	...
	entries.options.localGroupsMu = localGroupsMu
	if err != nil {
...
func (l *WithLock) lockGroupFile() (unlock func()) {
	l.options.localGroupsMu.Lock()
	return l.options.localGroupsMu.Unlock
}
