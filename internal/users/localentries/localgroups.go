...
// GroupManager is a struct that holds the current user groups and provides methods to
// retrieve and update them while ensuring that the system's user database is locked
// to prevent concurrent modifications.
type GroupManager struct {
	inputGroupFile  string
	outputGroupFile string
}
...
type options struct {
	// inputGroupFile is the path used to read the group file. Defaults to
	// [GroupFile], but can be overwritten in tests.
	inputGroupFile string
	// outputGroupFile is the path used to write the group file. Defaults to
	// [GroupFile], but can be overwritten in tests.
	outputGroupFile string
}

var defaultOptions = options{
	inputGroupFile:  GroupFile,
	outputGroupFile: GroupFile,
}

func NewGroupManager(args ...Option) GroupManager {
	if args != nil {
	...
	return GroupManager{
		inputGroupFile:  opts.inputGroupFile,
		outputGroupFile: opts.outputGroupFile,
	}
}

// GetEntries returns a copy of the current group entries.
func (g *GroupManager) GetEntries() (entries []types.GroupEntry, err error) {
	return parseLocalGroups(g.inputGroupFile)
}

// SaveEntries saves the provided group entries to the local group file.
func (g *GroupManager) SaveEntries(groups []types.GroupEntry) (err error) {
	defer decorate.OnError(&err, "could not save groups")

	inputPath := g.inputGroupFile
	groupPath := g.outputGroupFile
...
func (g *GroupManager) Update(username string, newGroups []string, oldGroups []string) (err error) {
	...
	allGroups, err := parseLocalGroups(g.inputGroupFile)
	if err != nil {
		return err
	}
	...
	return g.SaveEntries(allGroups)
}

func parseLocalGroups(groupFile string) (groups []types.GroupEntry, err error) {
	defer decorate.OnError(&err, "could not fetch existing local group")

	log.Debugf(context.Background(), "Reading groups from %q", groupFile)
