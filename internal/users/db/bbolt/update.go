...
func (c *Database) UpdateUserEntry(user UserDB, authdGroups []GroupDB, localGroups []string) error {
	...
	err := c.db.Update(func(tx *bbolt.Tx) error {
		...
		previousGroupsForCurrentUser, err := getFromBucket[userToGroupsDB](buckets[userToGroupsBucketName], user.UID)
		...
		if err := updateUser(buckets, user); err != nil {
			return err
		}
		...
		if err := updateUsersAndGroups(buckets, user.UID, authdGroups, previousGroupsForCurrentUser.GIDs); err != nil {
			return err
		}

		/* 4. Update user to local groups bucket */
		updateBucket(buckets[userToLocalGroupsBucketName], user.UID, localGroups)
