...
var schemaMigrations = []schemaMigration{
	{
		...
		migrate: func(m *Manager) error {
			...
			if err := renameUsersInUserGroupFile(oldNames, newNames); err != nil {
				return fmt.Errorf("failed to rename users in %s file: %w", userGroupFile, err)
			}
...
// renameUsersInUserGroupFile renames users in the /etc/group file.
func renameUsersInUserGroupFile(oldNames, newNames []string) error {
	...
	content, err := os.ReadFile(userGroupFile)
	if err != nil {
		return fmt.Errorf("error reading %s: %w", userGroupFile, err)
	}
	...
	for _, line := range oldLines {
		...
		if len(fields) != 4 {
			return fmt.Errorf("unexpected number of fields in %s line (expected 4, got %d): %s", userGroupFile, len(fields), line)
		}
	...
	if err := os.WriteFile(userGroupFile, []byte(strings.Join(newLines, "\n")), 0644); err != nil {
		return fmt.Errorf("error writing %s: %w", userGroupFile, err)
	}
