...
var schemaMigrations = []schemaMigration{
	{
		...
		migrate: func(m *Manager) error {
			...
			if err := renameUsersInGroupFile(oldNames, newNames); err != nil {
				return fmt.Errorf("failed to rename users in %s file: %w", groupFile, err)
			}
...
func renameUsersInGroupFile(oldNames, newNames []string) error {
	...
	for _, line := range oldLines {
		...
		if len(fields) != 4 {
			return fmt.Errorf("unexpected number of fields in %s line (expected 4, got %d): %s", groupFile, len(fields), line)
		}
