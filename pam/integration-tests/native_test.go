...
func TestNativeAuthenticate(t *testing.T) {
	...
	tapeCommand := fmt.Sprintf(nativeTapeBaseCommand, pam_test.RunnerActionLogin, socketPathEnv)
...
func TestNativeChangeAuthTok(t *testing.T) {
	...
	tapeCommand := fmt.Sprintf(nativeTapeBaseCommand, pam_test.RunnerActionPasswd, socketPathEnv)
		...
		"Change_password_successfully_and_authenticate_with_new_one": {
			...
			tapeVariables: map[string]string{
				"AUTHD_TEST_TAPE_LOGIN_COMMAND": fmt.Sprintf(
					nativeTapeBaseCommand, pam_test.RunnerActionLogin, socketPathEnv),
