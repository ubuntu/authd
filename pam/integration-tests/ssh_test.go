...
func testSSHAuthenticate(t *testing.T, sharedSSHD bool) {
	...
	prepareSSHTests := func(subtest *testing.T) {
		...
		pamMkHomeDirModule = buildSharedLibrary(t,
		...
		sshdPreloadLibrary := buildSharedLibrary(t, "Building sshd_preloader library", sources,
...
func (fw *filteredStderrWriter) Write(p []byte) (n int, err error) {
	debugLevel := strings.ToLower(os.Getenv("AUTHD_SSHD_STDERR_DEBUG_LEVEL"))
	if debugLevel == "" {
		debugLevel = "1"
	}
