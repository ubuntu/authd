...
func requireAuthdUser(t *testing.T, client authd.UserServiceClient, user string) *authd.User {
	...
	require.Equal(t, user, authdUser.Name, "Passwd user does not match")
...
func requireGetEntOutputEqualsPasswd(t *testing.T, getentOutput string, user *authd.User) {
	t.Helper()
...
func requireGetEntEqualsPasswd(t *testing.T, nssLibrary, authdSocket, user string, userPasswd *authd.User) {
	t.Helper()
