...
func NewUIModel(mTx pam.ModuleTransaction, clientType PamClientType, mode authd.SessionMode, require2FA bool, conn *grpc.ClientConn, exitStatus *PamReturnStatus) tea.Model {
	...
	m := newUIModelForClients(mTx, clientType, mode, require2FA, authd.NewPAMClient(conn), userServiceClient, exitStatus)
	m.conn = conn
	return m
}

// newUIModelForClients is the internal implementation of [NewUIModel] for testing purposes.
func newUIModelForClients(mTx pam.ModuleTransaction, clientType PamClientType, mode authd.SessionMode, require2FA bool, pamClient authd.PAMClient, userServiceClient authd.UserServiceClient, exitStatus *PamReturnStatus) uiModel {
	m := uiModel{
		pamMTx:      mTx,
	...
	m.authenticationModel = newAuthenticationModel(m.client, m.clientType, mode, require2FA)
